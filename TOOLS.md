...
- OpenClaw `v2026.4.12` live posture: the live container is now running `2026.4.12`; the isolated rehearsal lane on gateway `:19001` with Mission Control preview `3015/3016/3017` passed before the Apr 16 cutover. Install-path normalization was completed on Apr 17, so the canonical CLI path now resolves normally to `/data/.npm-global/bin/openclaw` at `2026.4.12` and the stale `/usr/local/bin/openclaw` `2026.3.8` shadow path has been removed.
...
- Autonomous Tasks sync posture:
  ...
  - Apr 11 generation/import behavior: daily-generated backlog tasks must land in the structured store as part of generation, not only in `AUTONOMOUS.md`; `Pull from md` is the explicit markdown-import lane, while `Apply board truth` reconciles markdown from the board
  - Apr 11 delete behavior hardening: manually deleted/suppressed legacy tasks should stay suppressed and must not be silently re-imported during normal sync/import flow unless they are genuinely regenerated
