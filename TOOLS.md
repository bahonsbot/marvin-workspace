...
- OpenClaw `v2026.4.12` live posture: the live container is now running `2026.4.12`; the isolated rehearsal lane on gateway `:19001` with Mission Control preview `3015/3016/3017` passed before the Apr 16 cutover. Operator CLI checks from this workspace should prefer `PATH=/data/.npm-global/bin:$PATH openclaw ...` until the split-install mismatch is normalized, because `/usr/local/bin/openclaw` can still resolve the stale `2026.3.8` binary.
