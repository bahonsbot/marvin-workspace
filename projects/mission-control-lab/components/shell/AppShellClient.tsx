...
export function AppShellClient({ children }: { children: ReactNode }) {
  ...
  const isHomeRoute = pathname === '/general/home' || pathname === '/home';
  const isTradingRoute = pathname.startsWith('/trading');
  ...
  const isCompactTopRoute = compactTopRoutes.has(pathname) || isTradingRoute;
  ...
  return (
    <div className={styles.appShellRoot}>
      ...
      <ToastRail />
      <div
        className={`app-shell-grid ${isChatRoute ? 'chat-shell-grid' : ''} ${isTasksRoute ? 'tasks-shell-grid' : ''} ${isAgentsRoute ? 'agents-shell-grid' : ''} ${isMobileWorkspaceRoute ? 'mobile-workspace-shell-grid' : ''} ${isHomeRoute ? 'home-shell-grid' : ''} ${isTradingRoute ? 'trading-shell-grid' : ''}`}
        style={{
      ...
      >
        ...
        <div className={styles.appShellContentColumn}>
          <main
            className={`app-shell-main ${styles.appShellMain} ${isChatRoute ? `${styles.chatShellMain} chat-shell-main` : ''} ${isTasksRoute ? 'tasks-shell-main' : ''} ${isAgentsRoute ? 'agents-shell-main' : ''} ${isMobileWorkspaceRoute ? 'mobile-workspace-shell-main' : ''} ${isHomeRoute ? 'home-shell-main' : ''} ${isTradingRoute ? 'trading-shell-main' : ''}`}
            style={{
