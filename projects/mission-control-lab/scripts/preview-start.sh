...
RUNTIME_DIR="${MISSION_CONTROL_PREVIEW_RUNTIME_DIR:-/data/.openclaw/workspace/projects/mission-control-lab/.lab-runtime}"
...
LIVE_PUBLIC_WS_PATH="${MISSION_CONTROL_WS_LIVE_PUBLIC_PATH:-/api/runtime-bridge/live}"
...
DEFEATBETA_SIDECAR_URL="${DEFEATBETA_SIDECAR_URL:-http://${DEFEATBETA_SIDECAR_HOST}:${DEFEATBETA_SIDECAR_PORT}}"
...
export MISSION_CONTROL_WS_LIVE_PUBLIC_PATH="$LIVE_PUBLIC_WS_PATH"
...
export DEFEATBETA_SIDECAR_HOST="$DEFEATBETA_SIDECAR_HOST"
export DEFEATBETA_SIDECAR_PORT="$DEFEATBETA_SIDECAR_PORT"
export DEFEATBETA_SIDECAR_URL="$DEFEATBETA_SIDECAR_URL"
...
spawn_detached "$RUNTIME_DIR/defeatbeta-sidecar.pid" "$RUNTIME_DIR/defeatbeta-sidecar.log" env PYTHONPATH="/data/.openclaw/workspace/projects/mission-control-lab/services/defeatbeta-probe/.python-packages" python3 ./services/defeatbeta-sidecar/app.py --host "$DEFEATBETA_SIDECAR_HOST" --port "$DEFEATBETA_SIDECAR_PORT"
...
for _ in {1..30}; do
  if ensure_alive "$RUNTIME_DIR/ws-sidecar.pid" && ensure_alive "$RUNTIME_DIR/piper-tts-worker.pid" && ensure_alive "$RUNTIME_DIR/moonshine-stt-worker.pid" && ensure_alive "$RUNTIME_DIR/defeatbeta-sidecar.pid" && ensure_alive "$RUNTIME_DIR/next.pid" && ensure_alive "$RUNTIME_DIR/latest.pid"; then
    if curl -fsS "http://127.0.0.1:${PREVIEW_PORT}/general/agents" >/dev/null 2>&1; then
      ...
      curl -fsS "${DEFEATBETA_SIDECAR_URL}/health" >/dev/null 2>&1 || true
...
echo "--- defeatbeta-sidecar.log ---" >&2
tail -n 40 "$RUNTIME_DIR/defeatbeta-sidecar.log" >&2 || true
exit 1
