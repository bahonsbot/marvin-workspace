...
kill_pid() {
  ...
  # Try the process group first, then always signal the direct PID as well.
  # Some Lab processes are wrapped by npm/setsid and the listener can survive
  # if only the process-group signal path is attempted.
...
kill_listeners_on_port() {
  ...
  if command -v ss >/dev/null 2>&1; then
    pids="$(ss -ltnp "( sport = :$port )" 2>/dev/null | grep -o 'pid=[0-9]\+' | cut -d= -f2 || true)"
  fi

  # Some container/runtime combinations expose the listener to fuser but not
  # through ss -p. Always merge fuser results when available so Lab restarts
  # remain port-scoped without falling back to broad process-name kills.
...
kill_from_pid_file "$DEFEATBETA_SIDECAR_PID_FILE"
kill_from_pid_file "$NEXT_PID_FILE"

# Lab safety: do not kill by broad process-name pattern. Dashboard uses the
# same script names, so lab cleanup is intentionally scoped to lab pid files
# and lab-only ports.
...
kill_listeners_on_port "$DEFEATBETA_SIDECAR_PORT"

rm -f "$PID_FILE" "$SIDECAR_PID_FILE" "$PIPER_TTS_PID_FILE" "$MOONSHINE_STT_PID_FILE" "$DEFEATBETA_SIDECAR_PID_FILE" "$NEXT_PID_FILE"
