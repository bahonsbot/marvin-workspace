...
class TradingProviderSmokeTests(unittest.TestCase):
    ...
    def test_watchlist_redesign_keeps_table_first_management_and_news_placeholders(self) -> None:
        ...
        self.assertIn("onUpdate?.(item._id, { priority", watchlist)
        self.assertIn("onUpdate?.(item._id, { alertLevel", watchlist)
        self.assertIn("Why is this on the list?", watchlist)
        self.assertIn("has-note", watchlist)
        ...
        self.assertIn("trading-watchlist-row-menu-actions", styles)
        ...
        self.assertIn("if (args.thesis !== undefined) patch.thesis", convex_watchlist)
