...
class TradingProviderSmokeTests(unittest.TestCase):
    ...
    def test_eodhd_dotted_symbol_resolution_uses_search_metadata(self) -> None:
        ...
        search_route = (ROOT / "app" / "api" / "trading" / "search" / "route.ts").read_text(encoding="utf-8")
        ...
        self.assertIn("findFinanceDatabaseProfile", search_route)
        self.assertIn("findFinanceDatabaseProfile(item.Code!.toUpperCase())", search_route)
        self.assertIn("sector: referenceProfile?.sector", search_route)
        self.assertIn("industry: referenceProfile?.industry", search_route)
    ...
    def test_analytics_page_is_valuation_workbench_without_portfolio_lens(self) -> None:
        ...
        self.assertIn("Read between the rows.", workbench)
        self.assertIn("<div className=\"trading-section-label\">Analytics</div>", workbench)
        self.assertIn("Generate a quick analysis or full thesis to get proper valuations and insights", workbench)
        self.assertIn("Valuation verdict", workbench)
        self.assertIn("Milou analysis", workbench)
        self.assertIn("Quick valuation", workbench)
        self.assertIn("Full thesis", workbench)
        self.assertIn("Quick analysis", workbench)
        self.assertIn("activeAnalysisTab", workbench)
        self.assertIn("openFullThesis", workbench)
        self.assertIn("generateFullThesis", workbench)
        self.assertIn("gatherFullThesisEvidence", workbench)
        self.assertIn("fetch('/api/trading/full-thesis'", workbench)
        self.assertIn("fetch('/api/trading/full-thesis/evidence'", workbench)
        self.assertIn("loadFullThesisEvidence", workbench)
        self.assertIn("Draft the investment case.", workbench)
        self.assertIn("Gather evidence", workbench)
        self.assertIn("Generate thesis", workbench)
        self.assertIn("Full Thesis route failed", workbench)
        self.assertIn("trading-analytics-full-thesis", workbench)
        self.assertIn("trading-analytics-evidence-pipeline", workbench)
        self.assertIn("Transcript catalogue", workbench)
        self.assertIn("LLM key data", workbench)
        self.assertIn("runFullThesisExtraction", workbench)
        self.assertIn("metric-changes", workbench)
        self.assertIn("forecast-drivers", workbench)
        self.assertIn("/api/trading/full-thesis/extract", workbench)
        self.assertIn("trading-analytics-extraction-preview", workbench)
        self.assertIn("Metric-change extraction", workbench)
        self.assertIn("Forecast-driver extraction", workbench)
        self.assertIn("Evidence gathered", workbench)
        self.assertIn("partial failures", workbench)
        self.assertIn("Economy context", workbench)
        self.assertNotIn("Thesis workspace placeholder", workbench)
        self.assertIn("trading-analytics-action-group", workbench)
        self.assertIn("CompanyLogo", workbench)
        self.assertIn("watchlist-metadata?symbols", workbench)
        self.assertIn("trading-analytics-company-logo", workbench)
        ...
        self.assertIn("fetch('/api/trading/benchmark-comparison'", workbench)
        ...
        self.assertIn("const [query, setQuery] = useState('')", workbench)
        self.assertIn("baseValue: null", workbench)
        self.assertIn("decisionZone: '—'", workbench)
        self.assertIn("emptyMethods", workbench)
        self.assertIn("emptyEvidence", workbench)
        self.assertIn("trading-analytics-corridor-empty", workbench)
        self.assertIn("trading-analytics-empty-chart", workbench)
        self.assertNotIn("baseValue: 161", workbench)
        self.assertNotIn("currentPrice: 149", workbench)
        self.assertIn("Watch / Buy weakness", workbench)
        self.assertNotIn("Validated match", workbench)
        self.assertNotIn("DefeatBeta: ${exchangeSymbolForDefeatBeta", workbench)
        ...
        self.assertIn("decisionZoneExplainerText", workbench)
        self.assertIn("decisionZone", workbench)
        self.assertNotIn("trading-analytics-status-copy", workbench)
        self.assertIn("trading-analytics-corridor-head", workbench)
        self.assertNotIn("Four models, one blended range", workbench)
        self.assertNotIn("Relative performance", workbench)
        self.assertNotIn("What the model reads", workbench)
        self.assertNotIn("Assumptions that matter", workbench)
        self.assertIn("Finance glossary", workbench)
        self.assertIn("BenchmarkOverlayChart", workbench)
        self.assertIn("Indexed performance · start = 100", workbench)
        self.assertIn("yfinance · 1Y indexed", workbench)
        self.assertIn("How to read this: a wider bar means that assumption can move fair value more", workbench)
        self.assertIn("Ask the stock expert", workbench)
        self.assertIn("Milou checks valuation work like a trading analyst", workbench)
        self.assertIn("stress-test the bull and bear case", workbench)
        self.assertIn("fetch('/api/trading/milou-analysis'", workbench)
        self.assertIn("askMilou", workbench)
        self.assertIn("Live answer", workbench)
        self.assertIn("MilouMarkdown", workbench)
        self.assertIn("parseInlineMarkdown", workbench)
        self.assertIn("milouUseWeb", workbench)
        self.assertIn("Use web search", workbench)
        self.assertNotIn("Use external web search when useful", workbench)
        self.assertIn("webAccessRequested: milouUseWeb ||", workbench)
        self.assertIn("rows={5}", workbench)
        self.assertIn("milouPromptsForSelection", workbench)
        self.assertIn("promptContextForSelection", workbench)
        self.assertIn("What pipeline or patent risk matters?", workbench)
        self.assertIn("What drives this ETF versus SPY/QQQ?", workbench)
        milou_route_source = (ROOT / "app" / "api" / "trading" / "milou-analysis" / "route.ts").read_text(encoding="utf-8")
        self.assertIn("agent:trading-advisor:main", milou_route_source)
        self.assertIn("chat.history", milou_route_source)
        self.assertIn("loadLatestMilouAnswer", milou_route_source)
        self.assertIn("GATEWAY_CALL_TIMEOUT_MS = 180_000", milou_route_source)
        self.assertIn("ANSWER_HISTORY_LIMIT = 50", milou_route_source)
        self.assertIn("30_000", milou_route_source)
        self.assertIn("registered trading-advisor runtime workspace", milou_route_source)
        full_thesis_route_source = (ROOT / "app" / "api" / "trading" / "full-thesis" / "route.ts").read_text(encoding="utf-8")
        self.assertIn("buildFullThesisPrompt", full_thesis_route_source)
        self.assertIn("agent:trading-advisor:main", full_thesis_route_source)
        self.assertIn("chat.history", full_thesis_route_source)
        self.assertIn("THESIS_HISTORY_LIMIT = 50", full_thesis_route_source)
        full_thesis_evidence_route = (ROOT / "app" / "api" / "trading" / "full-thesis" / "evidence" / "route.ts").read_text(encoding="utf-8")
        self.assertIn("getDefeatBetaTranscriptCatalog", full_thesis_evidence_route)
        self.assertIn("getDefeatBetaEconomyContext", full_thesis_evidence_route)
        self.assertIn("llmForecastDrivers", full_thesis_evidence_route)
        full_thesis_extract_route = (ROOT / "app" / "api" / "trading" / "full-thesis" / "extract" / "route.ts").read_text(encoding="utf-8")
        self.assertIn("buildFullThesisExtractionPrompt", full_thesis_extract_route)
        self.assertIn("getDefeatBetaTranscriptDetail", full_thesis_extract_route)
        self.assertIn("agent:trading-advisor:main", full_thesis_extract_route)
        full_thesis_extract_prompt = (ROOT / "lib" / "trading" / "full-thesis-extraction.ts").read_text(encoding="utf-8")
        self.assertIn("Extract key financial data", full_thesis_extract_prompt)
        self.assertIn("Every material claim should include paragraph references", full_thesis_extract_prompt)
        defeatbeta_sidecar = (ROOT / "services" / "defeatbeta-sidecar" / "app.py").read_text(encoding="utf-8")
        self.assertIn("/v1/ticker/{symbol}/transcript-catalog", defeatbeta_sidecar)
        self.assertIn("/v1/ticker/{symbol}/transcript", defeatbeta_sidecar)
        self.assertIn("build_transcript_detail", defeatbeta_sidecar)
        self.assertIn("/v1/economy/context", defeatbeta_sidecar)
        self.assertIn("summarize_key_financial_data_with_ai", defeatbeta_sidecar)
        self.assertIn("analyze_financial_metrics_change_for_this_quarter_with_ai", defeatbeta_sidecar)
        full_thesis_prompt_source = (ROOT / "lib" / "trading" / "full-thesis.ts").read_text(encoding="utf-8")
        self.assertIn("buildFullThesisPrompt", full_thesis_prompt_source)
        self.assertIn("Thesis verdict", full_thesis_prompt_source)
        self.assertIn("What would change my mind", full_thesis_prompt_source)
        self.assertIn("Watchlist plan", full_thesis_prompt_source)
        self.assertIn("Full Thesis evidence pipeline", full_thesis_prompt_source)
        self.assertIn("LLM forecast-driver analysis", full_thesis_prompt_source)
        self.assertIn("Yield curve", full_thesis_prompt_source)
        self.assertIn("not financial advice", full_thesis_prompt_source)
        self.assertIn("Do not pretend you searched if you did not", full_thesis_prompt_source)
        milou_prompt_source = (ROOT / "lib" / "trading" / "milou-analysis.ts").read_text(encoding="utf-8")
        self.assertIn("buildMilouAnalysisPrompt", milou_prompt_source)
        self.assertIn("Sensitivity factor", milou_prompt_source)
        self.assertIn("Web search was explicitly enabled by the user", milou_prompt_source)
        self.assertIn("first try available web/search tooling", milou_prompt_source)
        self.assertIn("Do not pretend you searched if you did not", milou_prompt_source)
        self.assertIn("Use Markdown sparingly", milou_prompt_source)
        ...
        self.assertIn("buildEvidence", valuation_model)
        self.assertIn("pointCount", valuation_model)
        self.assertIn("latestPointDate", valuation_model)
        self.assertIn("statementPointCount", valuation_model)
        ...
        benchmark_route = (ROOT / "app" / "api" / "trading" / "benchmark-comparison" / "route.ts").read_text(encoding="utf-8")
        benchmark_model = (ROOT / "lib" / "trading" / "benchmark-comparison.ts").read_text(encoding="utf-8")
        yfinance_prices = (ROOT / "lib" / "trading" / "sources" / "yfinance-prices.ts").read_text(encoding="utf-8")
        self.assertIn("buildBenchmarkComparison", benchmark_route)
        self.assertIn("fetchYfinancePriceHistory", benchmark_model)
        self.assertIn("yfinanceCandidates", benchmark_model)
        self.assertIn("resolvedSymbol", benchmark_model)
        self.assertIn("SPY", benchmark_model)
        self.assertIn("QQQ", benchmark_model)
        self.assertIn("trading_yfinance_prices.py", yfinance_prices)
        self.assertIn("Indexed to 100", benchmark_model)
        ...
        self.assertNotIn("Editable weights later", workbench)
        self.assertNotIn("Models unlock after analysis", workbench)
        self.assertNotIn("Select ticker first", workbench)
        self.assertNotIn("Evidence loads after analysis", workbench)
        self.assertNotIn("Assumptions pending", workbench)
        self.assertNotIn("SPY · QQQ · sector ETF", workbench)
        self.assertNotIn("DefeatBeta + providers", workbench)
        self.assertNotIn("<em>Bear/base/bull</em>", workbench)
        self.assertNotIn("Portfolio Lens", workbench)
        self.assertNotIn("Portfolio Impact", workbench)
        self.assertNotIn("trading-analytics-run-status", workbench)
        self.assertIn("trading-analytics-tab-status", workbench)
        self.assertIn("AnalyticsTabRow", workbench)
        self.assertIn("onSelect={setActiveAnalysisTab}", workbench)
        self.assertIn("trading-analytics-workbench", styles)
        self.assertIn("trading-analytics-milou", styles)
        self.assertIn("trading-analytics-milou-form", styles)
        self.assertIn("trading-analytics-chat-message-head", styles)
        self.assertIn("trading-analytics-milou-answer", styles)
        self.assertIn("font-weight: 560", styles)
        self.assertNotIn("font-weight: 720", styles)
        self.assertIn("trading-analytics-web-toggle", styles)
        self.assertIn("max-height: 420px", styles)
        ...
        self.assertIn("trading-analytics-company-logo", styles)
        self.assertIn("trading-analytics-action-group", styles)
        self.assertIn("trading-analytics-full-thesis", styles)
        self.assertIn("trading-analytics-thesis-actions", styles)
        self.assertIn("trading-analytics-thesis-output", styles)
        self.assertIn("trading-analytics-evidence-pipeline", styles)
        self.assertIn("trading-analytics-extraction-preview", styles)
        self.assertIn("trading-analytics-thesis-actions button.secondary", styles)
        self.assertIn("trading-tab-row-trailing", styles)
        self.assertIn("trading-analytics-tab-status", styles)
