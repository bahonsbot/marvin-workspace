...
const BOARDS: BoardMeta[] = [
  { id: 'autonomous', label: 'Autonomous', shortLabel: 'Auto', icon: '⚡' },
  { id: 'personal', label: 'Personal', shortLabel: 'Personal', icon: '🌱' },
...
function CompactSyncStatus({ state, details, onCleanup, cleanupBusy }: { state: 'unknown' | 'ok' | 'drift'; details: string; onCleanup: () => Promise<void>; cleanupBusy: boolean; }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', minWidth: 0, position: 'relative' }}>
      ...
      <button onClick={() => setConfirmOpen((v) => !v)} disabled={cleanupBusy} aria-label="Apply board truth to AUTONOMOUS.md" title="Rewrite AUTONOMOUS.md from the current board state" style={{ height: 26, padding: '0 10px', borderRadius: 999, border: '1px solid rgba(181,126,77,0.38)', background: state === 'drift' ? 'rgba(255,244,224,0.9)' : 'rgba(255,255,255,0.82)', color: cleanupBusy ? '#9a9a9a' : state === 'drift' ? '#8a5618' : '#5f655f', cursor: cleanupBusy ? 'progress' : 'pointer', fontSize: 11, fontWeight: 700, lineHeight: 1, textTransform: 'uppercase', letterSpacing: 0.04 }}>Apply board truth</button>
      {open ? <div style={{ position: 'absolute', top: 32, left: 0, zIndex: 4, maxWidth: 390, border: '1px solid rgba(200,195,188,0.45)', borderRadius: 14, background: 'rgba(255,253,251,0.98)', boxShadow: '0 12px 32px rgba(0,0,0,0.08)', padding: '10px 12px', fontSize: 11.5, color: '#6f726f', lineHeight: 1.55 }}><strong style={{ color: '#37413d' }}>{state === 'drift' ? 'Board and AUTONOMOUS.md disagree.' : 'Board and AUTONOMOUS.md are aligned.'}</strong><div style={{ marginTop: 4 }}>{details}</div></div> : null}
      {confirmOpen ? <div style={{ position: 'absolute', top: 32, left: 0, zIndex: 5, maxWidth: 420, border: '1px solid rgba(181,126,77,0.32)', borderRadius: 16, background: 'rgba(255,253,251,0.99)', boxShadow: '0 18px 42px rgba(0,0,0,0.12)', padding: '12px 14px', display: 'grid', gap: 10, color: '#4f4338' }}><div style={{ fontSize: 12, fontWeight: 800, color: '#8a5618', textTransform: 'uppercase', letterSpacing: 0.4 }}>Confirm board truth</div><div style={{ fontSize: 12.5, lineHeight: 1.55 }}>This rewrites AUTONOMOUS.md from the current Mission Control board. Use it only when the board is the intended source of truth.</div><div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap' }}><button type="button" onClick={() => setConfirmOpen(false)} style={{ padding: '7px 10px', borderRadius: 10, border: '1px solid rgba(200,195,188,0.45)', background: 'rgba(255,255,255,0.9)', color: '#5f655f', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>Cancel</button><button type="button" onClick={() => { setConfirmOpen(false); void onCleanup(); }} disabled={cleanupBusy} style={{ padding: '7px 10px', borderRadius: 10, border: 'none', background: '#0f1f19', color: '#fff', cursor: cleanupBusy ? 'progress' : 'pointer', fontSize: 12, fontWeight: 800 }}>{cleanupBusy ? 'Applying…' : 'Apply'}</button></div></div> : null}
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'grid', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div className="tasks-board-actions" style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => void onRefreshImport()} disabled={refreshBusy} style={{ minHeight: 26, padding: '5px 10px', borderRadius: 999, border: '1px solid rgba(200,195,188,0.45)', background: 'rgba(255,255,255,0.82)', color: refreshBusy ? '#9a9a9a' : '#5f655f', cursor: refreshBusy ? 'progress' : 'pointer', fontSize: 11, fontWeight: 700, lineHeight: 1.15, textTransform: 'uppercase', letterSpacing: 0.04, whiteSpace: 'normal', textAlign: 'center' }} aria-label="Import missing tasks from AUTONOMOUS.md" title="Import missing tasks from AUTONOMOUS.md without overwriting board-only tasks">{refreshBusy ? 'Importing…' : 'Import from AUTONOMOUS.md'}</button>
          <button onClick={onOpenNewTask} style={{ width: 40, height: 40, borderRadius: 999, border: 'none', background: '#0f1f19', color: '#fff', cursor: 'pointer', fontSize: 20, fontWeight: 700, boxShadow: '0 8px 22px rgba(15, 31, 25, 0.16)', flexShrink: 0 }} aria-label="New task" title="New task">+</button>
        </div>
      </div>
      <div className="tasks-board-scroll-shell" style={{ overflowX: 'auto', padding: '0 10px 8px 0' }}>
        <div className="tasks-board-scroll-hint" style={{ position: 'sticky', left: 0, width: 'fit-content', marginBottom: 8, padding: '5px 9px', borderRadius: 999, background: 'rgba(15, 31, 25, 0.08)', color: '#4f5d55', fontSize: 11, fontWeight: 700, zIndex: 2 }}>Scroll sideways to see all columns</div>
        <section className="tasks-board-grid tasks-autonomous-grid" style={{ gridTemplateColumns: 'repeat(5, minmax(218px, 1fr))', minWidth: 1120 }}>
...
export function TasksBoardSwitcher({ autonomousColumns, syncState, syncDetails, webResearchEnabled }: { autonomousColumns: Column[]; syncState: 'unknown' | 'ok' | 'drift'; syncDetails: string; webResearchEnabled?: boolean; }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 14 }}>
      <div className="tasks-board-tabs" role="tablist" aria-label="Task boards">{BOARDS.map(board => { const isActive = activeBoard === board.id; return <button key={board.id} role="tab" aria-selected={isActive} onClick={() => setActiveBoard(board.id)} className={isActive ? 'tasks-board-tab-active' : undefined}><span>{board.icon}</span><span className="tasks-board-tab-label">{board.label}</span><span className="tasks-board-tab-label-short">{board.shortLabel}</span></button>; })}</div>
