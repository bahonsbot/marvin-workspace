...
- Current product calibration (Mar 27):
  ...
  - Late Mar 30 / early Mar 31 Chat posture: Stop is now wired through the real runtime abort path, grouped tool bursts use a wider 10-second rolling window, and Recent Sessions entries are clickable session switches. Known follow-up: manual session switching can still reconnect too slowly and should be tightened later.
  - Mar 30 Chat reopen priorities: if Chat is reopened later, prioritize (1) effort/thinking source-of-truth readback, (2) recent-session switching/reconnect behavior, and only then lower-priority polish; do not reopen casually for broad redesign.
