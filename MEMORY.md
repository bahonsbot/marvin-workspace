...
- Apr 11 Link specialist posture: Link (`job-advisor`) now follows the same direct-specialist seat blueprint as Japin/Johan/Milou, with `agent:job-advisor:main`, a dedicated `agent-workspaces/job-advisor/` continuity workspace, a custom `job-advisor` skill, and `humanizer` added as a finishing-pass companion skill
