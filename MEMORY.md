...
- Apr 12 seat-bridge posture: Marvin now has a real Mission Control seat bridge. Current truthful routing is:
  ...
  - Japin / Johan / Milou / Link → persistent specialist seat sessions with seat-specific activation + continuity injection; in practice, their durable session logs may live under their matching agent storage roots (for example `job-advisor/sessions`) rather than only under the main-session storage tree
...
- Apr 15 specialist-workspace posture: keep `/data/.openclaw/workspace/agent-workspaces/<seat>/...` as the canonical specialist data layer. Seat-local roots like `/data/.openclaw/workspace-job-advisor` may still exist for runtime/bootstrap purposes, but they should alias their content paths (`memory`, `artifacts`, `.learnings`, `WORKSPACE.md`, `MEMORY.md`, `SKILLS.md`, shared `skills/`, shared `agent-workspaces/`) back to the canonical shared workspace rather than becoming a second drifting truth store.
- Apr 15 specialist-seat UX posture: specialist seats should feel present but brief — one short in-character acknowledgment when enough material is already present, one short verbal handoff on meaningful completion. Mission Control must hydrate both history and live transcript updates from the matching agent storage roots for these seats, not assume everything lives under the main-session storage tree.
