...
- Apr 7 Mission Control seat taxonomy posture: Vantage is a team lead like Sudo, not a standalone specialist; standalone specialist seats should reuse existing agent workspaces/skills, route to real distinct runtimes, and include specialist-scoped memory/continuity (Japin is the clearest lesson-memory benchmark)
- Apr 8 specialist-seat milestone: Japin (`language-tutor`) is now the first real standalone specialist seat in Mission Control, routing to `agent:language-tutor:main` with specialist-scoped continuity files and explicit post-lesson update discipline; Japin remains the benchmark pattern for future specialist seats
...
- Apr 9-10 specialist-seat expansion posture: Johan (`sportsbet-advisor`) and Milou (`trading-advisor`) now match the direct-specialist pattern rather than Marvin-routed specialist theater. Both have direct specialist runtime/session posture plus specialist-scoped continuity files, following the Japin benchmark
...
- Apr 11 Link specialist posture: Link (`job-advisor`) now follows the same direct-specialist seat blueprint as Japin/Johan/Milou, with `agent:job-advisor:main`, a dedicated `agent-workspaces/job-advisor/` continuity workspace, and provisional skill/avatar wiring to be finalized later
...
- Apr 8 Home posture: the accepted Mission Control Home page is now an editorial front door, not a widget dashboard — large serif greeting, compact weather/quote card, Market Watch as a real local RSS-derived headline reader with refresh, slimmer Current Tracks block, fixed bottom VPS/status strip, restored sidebar date, and explicitly no Daily Pulse / no quick-access grid / no dashboard-card mosaic
