...
- Current product calibration (Mar 27):
  ...
  - Apr 2 Chat runtime posture: transcript hydration in `useRuntimeBridge.ts` was hardened with signature-based rehydration so model/thinking/session changes do not get stuck in a tools-only view when transcript history exists.
